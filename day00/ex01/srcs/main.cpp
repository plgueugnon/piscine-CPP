...
int	add_cmd(Contact *contact)
{
	...
	prompt(YEL"New contact successfully added to the crappy phonebook"RST);
	return (1);
}
...
int	search_contact(Phonebook *pbk)
{
	...
	while (1)
	{
		...
		{
			error_prompt("Wrong input - the value you enter should be a digit between 1 and 8");
			std::cin.clear();
	...
	for(int i = 0; i < 8; i++)
	{
		...
		for(int j = 0; j < 2; j++)
		{
			if (pbk->contact[i].getstr(j).length() >= 10)
				std::cout << std::setw(9) << pbk->contact[i].getstr(j).substr(0, 9) << ".|";	
			else
				std::cout << std::setw(10) << pbk->contact[i].getstr(j) << "|";
		}
		if (pbk->contact[i].getstr(2).length() >= 10)
			std::cout << std::setw(9) << pbk->contact[i].getstr(2).substr(0,9) << "." << std::endl;
		else
			std::cout << std::setw(10) << pbk->contact[i].getstr(2) << std::endl;
	}
}
...
int	main(void)
{
	...
	while (std::getline(std::cin, cmd))
	{
		if ((cmd == "ADD" || cmd == "SEARCH" || cmd == "EXIT"))
		{
			...
			error_prompt("Wrong input - available commands are ADD, SEARCH or EXIT");
		prompt("Please enter a command");
	}
